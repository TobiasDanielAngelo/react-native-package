...
function interpolateNulls(data) {
    ...
    for (let i = 0; i < result.length; i++) {
        if (result[i] === null) {
            ...
            let prev = i - 1;
            while (prev >= 0 && result[prev] === null)
                prev--;
            let next = i + 1;
            while (next < result.length && result[next] === null)
                next++;
...
export function transformForTrendChart(data, traceKey, xAxis, yAxis, totalTitle, excludedFromTotal, noTotal) {
    ...
    data.forEach((item) => {
        ...
        if (!grouped[x])
            grouped[x] = {};
        grouped[x][trace] = y;
        if (!excludedFromTotal?.includes(trace))
            traces.add(trace);
    });
    ...
    if (!noTotal) {
        labels.forEach((label) => {
            ...
            const total = Object.entries(values).reduce((sum, [key, val]) => {
                if (excludedFromTotal?.includes(key))
                    return sum;
...
export function filterChartDataByFields(data, shownFields) {
    if (!data.legend || !data.datasets)
        return data;
...
export const useTrendChart = (data, traceKey, xKey, yKey, itemMap, excludedFromTotal, noTotal) => {
    ...
    const totalTitle = `Total${excludedFromTotal && excludedFromTotal.length > 0
        ...
        : ""}`;
    const { transformedData, legend } = transformForTrendChart(resolvedData, traceKey, xKey, yKey, totalTitle, excludedFromTotal, noTotal);
    useEffect(() => {
...
export const useCircleChart = (data, nameKey, dataKey, traceKey, itemMap) => {
